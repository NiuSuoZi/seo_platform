 ...
 * 后台权限管理
 */
class RuleController extends PlatformBaseController{
...
//******************权限***********************
    ...
    public function add(){
        ...
        if ($result) {
            $this->success('添加成功',U('Platform/Rule/index'));
        }else{
            $this->error('添加失败');
        }
    }
    ...
    public function edit(){
        ...
        if ($result) {
            $this->success('修改成功',U('Platform/Rule/index'));
        }else{
            $this->error('修改失败');
        }
    }
    ...
    public function delete(){
        ...
        if($result){
            $this->success('删除成功',U('Platform/Rule/index'));
        }else{
            $this->error('请先删除子权限');
        }
...
//*******************用户组**********************
    ...
    public function add_group(){
        ...
        if ($result) {
            $this->success('添加成功',U('Platform/Rule/group'));
        }else{
            $this->error('添加失败');
        }
    }
    ...
    public function edit_group(){
        ...
        if ($result) {
            $this->success('修改成功',U('Platform/Rule/group'));
        }else{
            $this->error('修改失败');
        }
    }
    ...
    public function delete_group(){
        ...
        if ($result) {
            $this->success('删除成功',U('Platform/Rule/group'));
        }else{
            $this->error('删除失败');
        }
    }
...
//*****************权限-用户组*****************
    ...
    public function rule_group(){
        if(IS_POST){
            ...
            $result=D('AuthGroup')->editData($map,$data);
            if ($result) {
                $this->success('操作成功',U('Platform/Rule/group'));
            }else{
                $this->error('操作失败');
            }
        }else{
...
//******************用户-用户组*******************
    ...
    public function add_user_to_group(){
        ...
        $this->success('操作成功',U('Platform/Rule/check_user',array('group_id'=>$data['group_id'],'username'=>$data['username'])));
    }
    ...
    public function delete_user_from_group(){
        ...
        if ($result) {
            $this->success('操作成功',U('Platform/Rule/admin_user_list'));
        }else{
            $this->error('操作失败');
        }
    }
    ...
    public function add_admin(){
        if(IS_POST){
            ...
            $result = $UsersModel->addData($data);
            ...
            if($result){
                ...
                // 操作成功
                $this->success('添加成功',U('Platform/Rule/admin_user_list'));
            }else{
    ...
    public function edit_admin(){
        if(IS_POST){
            ...
            if (!empty($data['password'])) {
                $HashLogin = $this->passwordHash($data['password']);
                $data['password'] = $HashLogin['password'];
                $data['salt'] = $HashLogin['salt'];
            }
            // p($data);die;
            //$result=D('Users')->editData($map,$data);
            $UserModel = new \Common\Model\UsersModel();
            $result = $UserModel->editData($map,$data);
            if($result){
                // 操作成功
                $this->success('编辑成功',U('Platform/Rule/edit_admin',array('id'=>$uid)));
            }else{
                $error_word=D('Users')->getError();
                if (empty($error_word)) {
                    $this->success('编辑成功',U('Platform/Rule/edit_admin',array('id'=>$uid)));
            ...
            $qrcode_format = sprintf("otpauth://totp/%s?secret=%s&issuer=%s",  'seo_platform' . ':' . $assign['user_data']['username'],$assign['user_data']['google_keys'],I('server.HTTP_HOST'));
            $this->assign('base64_images',$qrcode_format);
            $this->assign($assign);
            $this->display();
        }
    }
